...
class Processor {
  ...
  async addCrossLinksIncrementally() {
    ...
    const BATCH_SIZE = 10;
    const modifiedPaths = Array.from(this.modifiedPagesSinceLastCrossLink);
    let linksAdded = 0;

    for (let i = 0; i < modifiedPaths.length; i += BATCH_SIZE) {
      const batch = modifiedPaths.slice(i, i + BATCH_SIZE);
