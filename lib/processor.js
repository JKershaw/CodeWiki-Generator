...
const CrossLinkIndex = require('./cross-link-index');
...
class Processor {
  constructor(wikiPath = './dev-wiki') {
    ...
    // Track modified pages for incremental cross-linking
    this.modifiedPagesSinceLastCrossLink = new Set();

    // Track if pages were added/updated for incremental index generation
    this.modifiedPagesSinceLastIndex = new Set();

    // Inverted index for efficient cross-linking (O(k) instead of O(n²))
    this.crossLinkIndex = new CrossLinkIndex();

    // Performance timing (enabled via DEBUG_TIMING env var)
    this.perfTimer = new PerfTimer(process.env.DEBUG_TIMING === 'true');
  }
  ...
  async processConceptDocumentation(concept, analysis, summary) {
    ...
    if (existingPage) {
      ...
      // Track modification for incremental operations
      this.modifiedPagesSinceLastCrossLink.add(pagePath);
      this.modifiedPagesSinceLastIndex.add(pagePath);
  ...
  async processRepository(repoUrl, options = {}) {
    ...
    for (let i = startIndex; i < allCommits.length; i++) {
      ...
      // Process the commit (with timing)
      const commitSummary = await this.perfTimer.timeAsync(
        `commit-${i + 1}`,
        () => this.processCommit(commit, state)
      );
      ...
      // Regenerate wiki state - optimized to defer expensive operations
      // Architecture, guides, and index only run at the end for better performance
      // Cross-linking still runs periodically to keep incremental updates
      const isLastCommit = (i === allCommits.length - 1);
      let regenerated = false;

      // Expensive operations: only run on last commit (with timing)
      if (isLastCommit) {
        console.log(`\n🔄 Generating architecture overview (commit ${i + 1}/${allCommits.length})...`);
        await this.perfTimer.timeAsync(
          'architecture-overview',
          () => this.generateArchitectureOverview(repoInfo)
        );

        console.log(`\n📚 Generating guides (commit ${i + 1}/${allCommits.length})...`);
        await this.perfTimer.timeAsync(
          'guides',
          () => this.generateWikiGuides(repoInfo)
        );

        console.log(`\n📑 Generating wiki index (commit ${i + 1}/${allCommits.length})...`);
        await this.perfTimer.timeAsync(
          'wiki-index',
          () => this.generateWikiIndex(repoInfo)
        );

        regenerated = true;
      }

      // Cross-linking: run periodically based on frequency for incremental updates (with timing)
      if (config.shouldRunAgent('crossLinking', i, allCommits.length)) {
        console.log(`\n🔗 Adding cross-page links (commit ${i + 1}/${allCommits.length})...`);
        await this.perfTimer.timeAsync(
          'cross-linking',
          () => this.addCrossLinksToAllPages()
        );
        regenerated = true;
      }
      ...
      if (regenerated) {
        ...
        console.log(`⏭️  Skipping wiki regeneration (deferred until final commit)\n`);
      }
    }
    ...
    // Add cost and cache summary to stats
    const finalCostSummary = this.claudeClient.getCostSummary();
    stats.totalCost = finalCostSummary.totalCost;
    stats.cacheHits = finalCostSummary.cacheHits;
    stats.cacheMisses = finalCostSummary.cacheMisses;
    stats.cacheHitRate = finalCostSummary.cacheHitRate;

    // Print performance summary if timing is enabled
    this.perfTimer.printSummary();
  ...
  async generateWikiIndex(repoInfo) {
    try {
      // Skip index generation if no pages were modified
      if (this.modifiedPagesSinceLastIndex.size === 0) {
        console.log('No page changes since last index generation, skipping...');
        return;
      }

      console.log(`Regenerating index for ${this.modifiedPagesSinceLastIndex.size} modified pages...`);
      ...
      // Clear tracking set after successful generation
      this.modifiedPagesSinceLastIndex.clear();
  ...
  async addCrossLinksToAllPages() {
    try {
      // Use incremental cross-linking if we have tracked modifications
      if (this.modifiedPagesSinceLastCrossLink.size > 0) {
        console.log(`Adding cross-page links to ${this.modifiedPagesSinceLastCrossLink.size} modified pages...`);
        await this.addCrossLinksIncrementally();
      } else {
        console.log('No modified pages since last cross-linking, skipping...');
      }
  ...
  /**
   * Add cross-links only to pages that have been modified since last run
   * Uses inverted index to only update pages that actually mention modified pages
   * @private
   */
  async addCrossLinksIncrementally() {
    ...
    // Load full page data for all pages (needed for link discovery)
    const allPages = [];
    ...
    for (const pageMetadata of allPagesMetadata) {
      ...
      if (fullPage) {
        ...
        pagesByPath.set(fullPage.path, fullPage);

        // Update inverted index with this page
        this.crossLinkIndex.updatePage(
          fullPage.path,
          fullPage.content,
          fullPage.metadata.title || fullPage.path
        );
      }
    }

    // Find all pages that mention any of the modified pages using the inverted index
    // This is much faster than scanning all pages (O(k) instead of O(n²))
    const modifiedPageTitles = [];
    for (const modifiedPath of this.modifiedPagesSinceLastCrossLink) {
      const page = pagesByPath.get(modifiedPath);
      if (page) {
        modifiedPageTitles.push(page.metadata.title || page.path);
      }
    }

    const pagesToUpdate = this.crossLinkIndex.getPagesToUpdateForMultiple(modifiedPageTitles);

    // Also include the modified pages themselves
    for (const modifiedPath of this.modifiedPagesSinceLastCrossLink) {
      pagesToUpdate.add(modifiedPath);
    }

    console.log(`Cross-linking: ${this.modifiedPagesSinceLastCrossLink.size} pages modified, ${pagesToUpdate.size} pages need updates`);

    // Process pages in parallel batches of 10
    const BATCH_SIZE = 10;
    const pathsToUpdate = Array.from(pagesToUpdate);
    let linksAdded = 0;

    for (let i = 0; i < pathsToUpdate.length; i += BATCH_SIZE) {
      const batch = pathsToUpdate.slice(i, i + BATCH_SIZE);
      ...
      const results = await Promise.all(
        batch.map(async (pagePath) => {
          ...
          if (!page || !page.content) {
            return { success: false }; // Skip if page not found or has no content
          }
