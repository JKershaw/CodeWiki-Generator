...
const TechDebtAnalysisAgent = require('./agents/tech-debt-analysis-agent');
const SecurityAnalysisAgent = require('./agents/security-analysis-agent');
...
class Processor {
  constructor(wikiPath = './dev-wiki') {
    ...
    this.techDebtAnalysisAgent = new TechDebtAnalysisAgent();
    this.securityAnalysisAgent = new SecurityAnalysisAgent();
  ...
  async processCommit(commit, state) {
    ...
    for (const file of commit.files) {
      ...
      const techDebtFindings = await this.techDebtAnalysisAgent.analyze(
        file.filename,
        file.patch,
        analysis
      );

      // Process tech debt findings if any
      if (techDebtFindings.items && techDebtFindings.items.length > 0) {
        await this.processQualityDocumentation(techDebtFindings, summary);
      }

      // Analyze for security issues
      const securityFindings = await this.securityAnalysisAgent.analyze(
        file.filename,
        file.patch,
        analysis
      );

      // Process security findings if any
      if (securityFindings.findings && securityFindings.findings.length > 0) {
        await this.processSecurityDocumentation(securityFindings, summary);
      }
  ...
  /**
   * Process quality documentation (tech debt findings)
   * @param {Object} findings - Tech debt findings from TechDebtAnalysisAgent
   * @param {Object} summary - Processing summary to update
   */
  async processQualityDocumentation(findings, summary) {
    // Process each tech debt item
    for (const item of findings.items) {
      ...
      const pagePath = `quality/${item.severity}-${sanitizedTitle}.md`;
...
## Related
- [${findings.filePath}](../${this._relativePathToComponent(findings.filePath)})
`;
      ...
      if (existingPage) {
        // Update if changed
        await this.wikiManager.updatePage(pagePath, content, {
          title: item.title,
          category: 'quality',
          severity: item.severity,
          type: item.type,
          sourceFile: findings.filePath
        });
        summary.pagesUpdated++;
      } else {
        // Create new page
        await this.wikiManager.createPage(pagePath, content, {
          title: item.title,
          category: 'quality',
          severity: item.severity,
          type: item.type,
  ...
  /**
   * Process security documentation (security findings)
   * @param {Object} findings - Security findings from SecurityAnalysisAgent
   * @param {Object} summary - Processing summary to update
   */
  async processSecurityDocumentation(findings, summary) {
    // Process each security finding
    for (const finding of findings.findings) {
      // Create page name from title
      const sanitizedTitle = finding.title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '');

      const pagePath = `security/${finding.severity}-${sanitizedTitle}.md`;

      // Format content
      const cweLine = finding.cwe ? `\n## CWE\n${finding.cwe}` : '';
      const owaspLine = finding.owasp ? `\n## OWASP Category\n${finding.owasp}` : '';
...
`;
  ...
  /**
   * Convert file path to relative component path
   * @private
   */
  _relativePathToComponent(filePath) {
    // lib/agents/code-analysis-agent.js -> components/code-analysis-agent.md
    const fileName = filePath.split('/').pop().replace(/\.(js|ts|py|java|go)$/, '');
    const kebab = fileName.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
    return `components/${kebab}.md`;
  }
  ...
  async processRepository(repoUrl, options = {}) {
    ...
    if (!stats.stopped || stats.stopReason !== 'cost_limit') {
      ...
      // Add cross-page links to all pages
  ...
  async addCrossLinksToAllPages() {
    try {
      ...
      for (const pageMetadata of allPagesMetadata) {
        const fullPage = await this.wikiManager.getPage(pageMetadata.path);
        if (fullPage) {
