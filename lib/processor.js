...
class Processor {
  ...
  async processCommit(commit, state) {
    ...
    for (const file of commit.files) {
      ...
      // Check if file is a meta-document (philosophy, specification, etc.)
      const metaInfo = this.isMetaDocument(file.filename);
      if (metaInfo) {
        await this.processMetaDocument(
          file.filename,
          metaInfo.category,
        ...
        summary.metaDocumentsProcessed++;
        continue; // Skip regular code processing for meta documents
      }
   ...
   * @param {Object} summary - Processing summary to update
   */
  async processMetaDocument(sourceFilePath, category, documentType, title, summary) {
    try {
      ...
      // Ingest the document
      const result = await this.metaDocumentIngestionAgent.ingestDocument(
        sourceFilePath,
        category,
        documentType,
        existingPages
      ...
      if (existingPage) {
        ...
        await this.wikiManager.updatePage(pagePath, result.content, {
          title: title,
          category: category,
        ...
        // Create new page
        await this.wikiManager.createPage(pagePath, result.content, {
          title: title,
          category: category,
          sourceFile: sourceFilePath,
          created: new Date().toISOString().split('T')[0]
      ...
      // Don't throw - gracefully continue with other files
    }
  }
  ...
  async processRepository(repoUrl, options = {}) {
    ...
    for (let i = startIndex; i < allCommits.length; i++) {
      ...
      // Regenerate complete wiki state after each commit
      // This ensures each commit produces a complete, self-contained wiki
      console.log(`\n🔄 Regenerating wiki state after commit ${i + 1}/${allCommits.length}...`);
      ...
      console.log(`✓ Wiki state complete\n`);
    }
