...
class Config {
  constructor() {
    ...
    // Historical commit optimization
    // Only run cross-linking on the last N commits (0 = all commits)
    this.crossLinkingRecentCommits = parseInt(process.env.CROSS_LINKING_RECENT_COMMITS || '10', 10);
  ...
  shouldRunAgent(agentName, commitIndex, totalCommits) {
    ...
    // Special handling for cross-linking: skip historical commits
    if (agentName === 'crossLinking' && this.crossLinkingRecentCommits > 0) {
      const commitsFromEnd = totalCommits - commitIndex;
      // Only run if within the last N commits
      if (commitsFromEnd > this.crossLinkingRecentCommits) {
        return false;
      }
    }
