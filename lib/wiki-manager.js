...
class WikiManager {
   ...
   */
  async updatePage(filePath, content, newMetadata = {}) {
    const fullPath = path.join(this.wikiPath, filePath);

    // Get existing page or create new metadata
    let existingMetadata = {};
    const existingPage = await this.getPage(filePath);
    if (existingPage) {
  ...
  async updateMetadata(filePath, newMetadata = {}) {
    ...
    const fullPath = path.join(this.wikiPath, filePath);
    const fileContent = this._serializePage(metadata, page.content);
    await fs.writeFile(fullPath, fileContent, 'utf-8');
  }
