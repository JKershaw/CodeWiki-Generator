...
class WikiManager {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          // Skip _history directories to avoid processing archived versions
          if (entry.name === '_history') {
            continue;
          }
  ...
  async _archivePageVersion(filePath, commitInfo) {
    // Guard: Never archive files that are already in _history to prevent infinite nesting
    if (filePath.includes('_history')) {
      return;
    }
