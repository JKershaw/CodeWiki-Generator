...
class ClaudeClient {
  constructor(apiKey = null) {
    ...
    // Default model from config (can be overridden via CLAUDE_MODEL env var)
    ...
    // Concurrency control
    this.maxConcurrent = parseInt(process.env.CLAUDE_MAX_CONCURRENT || '5', 10);
    this.activeRequests = 0;
    this.requestQueue = [];

    // Response caching (LRU cache)
    this.cacheEnabled = process.env.CLAUDE_CACHE_ENABLED !== 'false'; // Default: enabled
    this.cache = new Map(); // key = hash, value = { response, timestamp, hits }
    this.cacheMaxSize = parseInt(process.env.CLAUDE_CACHE_MAX_SIZE || '1000', 10);
    this.cacheHits = 0;
    this.cacheMisses = 0;

    // Pricing (per million tokens)
    this.pricing = {
      'claude-haiku-4-5-20251001': { input: 1.0, output: 5.0 },  // Latest model (Oct 2025)
      'claude-haiku-4-5': { input: 1.0, output: 5.0 },  // Alias for latest Haiku
  ...
  _getCacheKey(prompt, options) {
    ...
    const key = JSON.stringify({
      ...
      system: options.system || options.systemPrompt || null,
      // Don't include maxTokens in cache key (different token limits can use same response)
    });
  ...
  async sendMessage(prompt, options = {}) {
    ...
    if (!tools) {
      ...
      if (cachedResponse) {
        // Return cached response (text format for backward compatibility)
        return cachedResponse.content[0].text;
      }
    }

    // Queue the request to respect concurrency limits
    ...
    // Cache the response (only for non-tool requests)
    if (!tools) {
      const cacheKey = this._getCacheKey(prompt, options);
  ...
  async sendMessageJSON(prompt, options = {}) {
    ...
    // Check for markdown code block: ```json ... ``` (with or without closing fence)
    const codeBlockMatch = jsonText.match(/```(?:json)?\s*\n([\s\S]*?)(?:\n```|$)/);
    if (codeBlockMatch) {
      jsonText = codeBlockMatch[1].trim();
    } else if (jsonText.startsWith('```')) {
      // Handle edge case where response starts with ``` but no newline
  ...
  /**
   * Queue a request to respect concurrency limits
   * @private
   */
