...
class CodeAnalysisAgent {
  constructor() {
    ...
    this.maxTokens = 4000;  // Increased from 2000 to handle larger analyses
  ...
  _validateResponse(response) {
    ...
    // Get file path from first concept if available (for legacy format)
    const defaultFilePath = concepts[0]?.sourceFile || 'Not specified';
    ...
    concepts = concepts.map(concept => ({
      ...
      sourceFile: concept.sourceFile || defaultFilePath
    }));
