...
The system integrates a **Wiki Index Agent** component into the main processor following the established multi-agent architecture pattern. This agent automatically generates navigation indexes after the primary documentation generation is complete.

Key features include:

- **Conditional Post-Processing**: Wiki index generation is skipped when processing stops due to cost limits, implementing a cost-aware completion pattern
...
- **Processor** (`lib/processor.js`): Main integration point where the Wiki Index Agent is instantiated and executed
