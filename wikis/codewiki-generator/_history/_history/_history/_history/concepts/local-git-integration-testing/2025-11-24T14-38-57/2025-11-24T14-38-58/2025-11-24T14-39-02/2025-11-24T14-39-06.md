...
`test-run-local.js` provides a self-contained testing pattern that runs the CodeWiki Processor against your own repository's git history rather than external repositories. This enables validation of the processor on real-world commit data without external API dependencies, rate limits, or network calls—making it ideal for development, debugging, and pre-deployment verification.
